...
*submodules*. *lang* is the source language of the library (if not specified,
this will use the [default language](#project) for the project); this will
affect how the package is resolved. For native libraries (C, C++, Fortran, etc),
this will use `mopack` to resolve the package; otherwise, this will use a
language-specific check to find libraries on the system. If this function is
