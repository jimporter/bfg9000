...
class TestUserArgs(IntegrationTest):
    def __init__(self, *args, **kwargs):
        super().__init__(os.path.join(examples_dir, '12_custom_args'),
    ...
    def test_dist(self):
        dist = output_file('12_custom_args.tar.gz')
        self.configure()
        self.build('dist')
        self.assertExists(dist)
        with tarfile.open(self.target_path(dist)) as t:
            self.assertEqual(set(t.getnames()), {
                '12_custom_args/build.bfg',
                '12_custom_args/options.bfg',
                '12_custom_args/simple.cpp',
            })
